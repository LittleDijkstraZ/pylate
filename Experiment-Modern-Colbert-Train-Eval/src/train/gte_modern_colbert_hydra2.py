...
def build_run_name(cfg: DictConfig) -> str:
    ...
    base = (
        ...
        # Append weight decay if provided
...
@hydra.main(version_base=None, config_path="../../conf2", config_name="gte_modern_colbert")
def main(cfg: DictConfig) -> None:
    ...
    try:
        gc_enabled = bool(getattr(cfg, "gradient_checkpointing", False))
    except Exception:
        gc_enabled = False
    if gc_enabled:
        try:
            ...
            # Some transformer configs require disabling use_cache when using gradient checkpointing
            if auto_model is not None and hasattr(auto_model, "config") and hasattr(auto_model.config, "use_cache"):
                auto_model.config.use_cache = False
        except Exception:
            # Be permissive: if enabling fails, continue with standard training
            pass
    ...
    # Optional weight decay if provided in config
    try:
        wd = cfg.get("weight_decay")
    except Exception:
        wd = None
    if wd is not None:
        args_kwargs["weight_decay"] = wd
